...
def denoising_search_batch(quene_msms, quene_pmz, reference_lib, identitiy_search_mass_error = 0.01, max_allowed_denoising_tolerance=0.005, 
    ...
    with mp.Pool(processes=6) as pool:
        ...
        results = pool.starmap(denoising_search_single, tqdm([(msms, pmz, reference_lib, identitiy_search_mass_error, max_allowed_denoising_tolerance, 
                            pmz_col, smiles_col, adduct_col,  msms_col) for msms, pmz in zip(quene_msms, quene_pmz)], total=len(quene_msms)))
    return results



def denoising_search_single(msms, pmz, reference_lib, identitiy_search_mass_error=0.01, max_allowed_denoising_tolerance = 0.005, 

...
mrm_validated = read_df(os.path.join(master_dir, 'mrm_validated_updatedd.csv'))
processed_formulas = []
for index, row in mrm_validated.iterrows():
    processed_formulas.append(drf.prep_formula(row['reference_smiles'], row['reference_adduct']))
mrm_validated['processed_formulas']=processed_formulas
